...
const route = await fetch(`${expected.url}/connection-check`, { signal: AbortSignal.timeout(30_000) });
if (!route.ok || !(await route.text()).includes('<html')) throw new Error('Hosting must serve the SPA for a direct route.');

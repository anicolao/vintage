...
const event = (id, type = 'listing/created', payload = { title: 'Linen jacket' }, extra = {}) => ({ id, streamId:'draft', actorUid:'alice', deviceId:'device', clientSeq:1, correlationId:id, causationId:null, createdAt:{seconds:1,nanoseconds:0}, schemaVersion:2, reducerVersion:1, streamVersion:1, type, payload, ...extra });
test('replay is pure, ordered, idempotent and upgrades old context payloads', () => {
  const created = event('a'); const context = event('b','context/changed',{text:'Old schema, preserved'}, { schemaVersion:1, streamVersion:2 });
  const input = [context,created,context]; const original = structuredClone(input);
  const state = reduceListing(input,'draft','alice');
  assert.equal(state.title,'Linen jacket'); assert.equal(state.context,'Old schema, preserved'); assert.equal(state.version,2);
...
test('pending state never advances acknowledged command version and context requires creation', () => {
  const pending=event('b','context/changed',{context:'Pending'}, {createdAt:null, streamVersion:2});

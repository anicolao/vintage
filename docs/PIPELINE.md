...
Every field edit checks the previously displayed field value as well as workflow version. Example edits compare the previous example set. A conflict preserves local edits and explains how to review the latest version; choosing that action explicitly discards the unconfirmed queue. Other accounts never see it. Offline sign-out counts pipeline work among device-only changes.

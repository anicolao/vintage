...
Install backend dependencies with `npm ci --prefix functions`. `npm run test:e2e` now owns the Functions emulator as well as Auth/Firestore/Storage. Deployed Functions use Vertex AI through the dedicated runtime service account. Run `node scripts/configure-pipeline.mjs` to enable its API and grant the runtime role. The deterministic AI server is test-only and cannot be selected by a live project. See [Pipeline implementation](./PIPELINE.md).
